  ...
  {/if}<!--1 end-->
    ...
    {else}<!--1 else-->
      <div class="margin-bottom">{$challenge}</div>
      <div class="panel panel-info">
        <div class="panel-heading"><h3 class="panel-title">{$LANG.twofacantaccess2ndfactor}</h3></div>
        <div class="panel-body"><a href="clientarea.php?backupcode=1">{$LANG.twofaloginusingbackupcode}</a></div>
      </div>
    {/if}<!--1 end-->
    </form>
  </div>
