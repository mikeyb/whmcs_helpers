{foreach from=$issues item=issue}
  <div class="panel {if $issue.clientaffected}panel-warning{else}panel-info{/if}">
    <div class="panel-heading">{$issue.title} ({$issue.status})</div>
      <ul class="list-group">
      ...
      {if $issue.server or $issue.affecting}<!--1-->
        <li class="list-group-item"><strong>{$LANG.networkissuesaffecting} {$issue.type}</strong> - 
        {if $issue.type eq $LANG.networkissuestypeserver}<!--2-->
          {$issue.server}
        {else}<!--2 else-->
          {$issue.affecting}
        {/if}<!--2 end-->
      ...
      {/if}<!--1 end-->
        <li class="list-group-item"><p>{$issue.description}</p></li>
  ...
  <div class="table-responsive">
    <table class="table table-striped">
      ...
      {foreach from=$servers key=num item=server}
        <tr>
          <td>{$server.name}</td>
          <td class="text-center" id="port80_{$num}"><span class="fa fa-spinner fa-spin"></span></td>
          <td class="text-center" id="port21_{$num}"><span class="fa fa-spinner fa-spin"></span></td>
          <td class="text-center" id="port110_{$num}"><span class="fa fa-spinner fa-spin"></span></td>
          <td class="text-center"><a href="{$server.phpinfourl}" target="_blank">{$LANG.serverstatusphpinfo}</a></td>
          <td class="text-center" id="load{$num}"><span class="fa fa-spinner fa-spin"></span></td>
      ...
      {foreachelse}
        <tr><td colspan="7">{$LANG.serverstatusnoservers}</td></tr>
      {/foreach}
      </tbody>
    </table>
  </div>
