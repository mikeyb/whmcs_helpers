...
{if !$invalidTicketId}<!--1-->
  <div class="panel panel-info panel-collapsable{if !$postingReply} panel-collapsed{/if} hidden-print">
    <div class="panel-heading" id="ticketReply">
      <div class="collapse-icon pull-right"><i class="fa fa-{if !$postingReply}plus{else}minus{/if}"></i></div>
      <h3 class="panel-title"><i class="fa fa-pencil"></i> &nbsp; {$LANG.supportticketsreply}</h3>
    </div>
    <div class="panel-body{if !$postingReply} panel-body-collapsed{/if}">
      <form method="post" action="{$smarty.server.PHP_SELF}?tid={$tid}&amp;c={$c}&amp;postreply=true" enctype="multipart/form-data" role="form" id="frmReply">
        ...
        <div class="row form-group">
          ...
          <div class="col-xs-12 ticket-attachments-message text-muted">{$LANG.supportticketsallowedextensions}: {$allowedfiletypes}</div>
        </div>
...
{foreach from=$descreplies key=num item=reply}
  <div class="ticket-reply{if $reply.admin} staff{/if}">
    ...
    <div class="user">
      ...
      {elseif $reply.contactid}<!--2 elseif-->
        {$LANG.supportticketscontact}
      {elseif $reply.userid}<!--2 elseif-->
    ...
    <div class="message">
      {$reply.message}
    {if $reply.id && $reply.admin && $ratingenabled}<!--2-->
      <div class="clearfix">
      {if $reply.rating}<!--3-->
