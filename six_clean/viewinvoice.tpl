...
<body>
  ...
  {if $invalidInvoiceIdRequested}<!--1-->
  ...
  {else}<!--1 else-->
    <div class="row">
      ...
      <div class="col-sm-5 text-center">
        ...
        {if $status eq "Unpaid"}<!--2-->
          <span class="unpaid">{$LANG.invoicesunpaid}</span>
        {elseif $status eq "Paid"}<!--2 elseif-->
          <span class="paid">{$LANG.invoicespaid}</span>
        {elseif $status eq "Refunded"}<!--2 elseif-->
          <span class="refunded">{$LANG.invoicesrefunded}</span>
        {elseif $status eq "Cancelled"}<!--2 elseif-->
          <span class="cancelled">{$LANG.invoicescancelled}</span>
        {elseif $status eq "Collections"}<!--2 elseif-->
          <span class="collections">{$LANG.invoicescollections}</span>
        {/if}<!--2 end-->
        </div>
      {if $status eq "Unpaid"}<!--2-->
        <div class="small-text">{$LANG.invoicesdatedue}: {$datedue}</div>
        <div class="payment-btn-container" align="center">{$paymentbutton}</div>
      {/if}<!--2 end-->
      </div>
    </div>
    <hr>
  ...
  {/if}<!--2 end-->
    ...
    <div class="row">
      <div class="col-sm-6 pull-sm-right text-right-sm">
        ...
        <address class="small-text">{$payto}</address>
      </div>
      <div class="col-sm-6">
...
{/if}<!--2-->
    ...
    <div class="row">
      <div class="col-sm-6">
        ...
        {if $status eq "Unpaid" && $allowchangegateway}<!--2-->
    ...
    <div class="transactions-container small-text">
      <div class="table-responsive">
        <table class="table table-condensed">
          ...
          {foreachelse}
            <tr><td class="text-center" colspan="4">{$LANG.invoicestransnonefound}</td></tr>
          {/foreach}
            <tr>
