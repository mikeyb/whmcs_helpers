  ...
  <script type="text/javascript">
    ...
    {if $orderby == 'product'}<!--1-->
      table.order([0, '{$sort}'], [3, 'asc']);
    {elseif $orderby == 'amount' || $orderby == 'billingcycle'}<!--1 elseif-->
      table.order(1, '{$sort}');
    {elseif $orderby == 'nextduedate'}<!--1 elseif-->
      table.order(2, '{$sort}');
    {elseif $orderby == 'domainstatus'}<!--1 elseif-->
      table.order(3, '{$sort}');
    {/if}<!--1 end-->
      table.draw();
    });
      ...
      {foreach key=num item=service from=$services}
        <tr onclick="clickableSafeRedirect(event, 'clientarea.php?action=productdetails&amp;id={$service.id}', false)">
          ...
          <td class="responsive-edit-button" style="display: none;">
            <a href="clientarea.php?action=productdetails&amp;id={$service.id}" class="btn btn-block btn-info">{$LANG.manageproduct}</a>
          </td>
        </tr>
      {/foreach}
      </tbody>
