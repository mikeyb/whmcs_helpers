  ...
  <script type="text/javascript">
    ...
    {if $orderby == 'default'}<!--1-->
      table.order([4, 'desc'], [2, 'asc']);
    {elseif $orderby == 'invoicenum'}<!--1 elseif-->
      table.order(0, '{$sort}');
    {elseif $orderby == 'date'}<!--1 elseif-->
      table.order(1, '{$sort}');
    {elseif $orderby == 'duedate'}<!--1 elseif-->
      table.order(2, '{$sort}');
    {elseif $orderby == 'total'}<!--1 elseif-->
      table.order(3, '{$sort}');
      ...
      {foreach key=num item=invoice from=$invoices}
        <tr onclick="clickableSafeRedirect(event, 'viewinvoice.php?id={$invoice.id}', false)">
          ...
          <td class="responsive-edit-button" style="display: none;">
            <a href="viewinvoice.php?id={$invoice.id}" class="btn btn-block btn-info">{$LANG.manageproduct}</a>
          </td>
        </tr>
      {/foreach}
      </tbody>
