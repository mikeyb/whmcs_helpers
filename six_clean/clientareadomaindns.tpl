...
{else}<!--1 else-->
  <form class="form-horizontal" method="post" action="{$smarty.server.PHP_SELF}?action=domaindns">
    ...
    <table class="table table-striped">
      ...
      {foreach from=$dnsrecords item=dnsrecord}
        <tr>
          ...
          <td>
          {if $dnsrecord.type eq "MX"}<!--2-->
            <input type="text" name="dnsrecordpriority[]" value="{$dnsrecord.priority}" size="2" class="form-control">
          {else}<!--2 else-->
    ...
    <p class="text-right"> * {$LANG.domaindnsmxonly}</p>
    <p class="text-center">
